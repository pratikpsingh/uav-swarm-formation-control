# Policy-compression smoke verification

Date: 2026-09-07. Profile: `smoke`. Method:
`validated-policy-compression`.
...
trained every student, performed closed-loop evaluation across all 16 neighbor-study conditions, exported
...
The neighbor-study smoke teacher correctly failed the research gate: collision-free success was zero on both
...
the ignored legacy run at `artifacts/deployment/stage12-smoke-20260907/smoke/policy-compression/`; it is not renamed because its manifest preserves the executed identity.

The scientific gate remains open until a full neighbor-study policy passes the predeclared teacher metrics,
